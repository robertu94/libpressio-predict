...
list5d=[]
list5m=[]
with open('sz_libpressioSCALE.csv','w',newline='') as csvfile:
    ...
    i=0
    while i<30:
        process=subprocess.run(["/usr/bin/time","pressio","-i","V-98x1200x1200.dat","-m","time","-M","all","-d","141120000","-t","float","sz","-o","sz:abs_err_bound=0.00995761","-o","sz:error_bound_mode_str=abs"],capture_output=True,text=True)
    ...
    i=0
    while i<30:
        process=subprocess.run(["/usr/bin/time","pressio","-i","V-98x1200x1200.dat","-m","time","-M","all","-d","141120000","-t","float","sz","-o","sz:abs_err_bound=0.995761","-o","sz:error_bound_mode_str=abs"],capture_output=True,text=True)
        print(process.stdout)
        print(process.stderr)
        m=re.search('time:compress_many <uint32> = (\d+)',process.stderr)
        p=re.search('time:decompress_many <uint32> = (\d+)',process.stderr)
    ...
    i=0
    while i<30:
        process=subprocess.run(["/usr/bin/time","pressio","-i","V-98x1200x1200.dat","-m","time","-M","all","-d","141120000","-t","float","sz","-o","sz:abs_err_bound=1.991522","-o","sz:error_bound_mode_str=abs"],capture_output=True,text=True)
        print(process.stdout)
        print(process.stderr)
        m=re.search('time:compress_many <uint32> = (\d+)',process.stderr)
        p=re.search('time:decompress_many <uint32> = (\d+)',process.stderr)
    ...
    i=0 
    while i<30:
        process=subprocess.run(["/usr/bin/time","pressio","-i","V-98x1200x1200.dat","-m","time","-M","all","-d","141120000","-t","float","sz","-o","sz:abs_err_bound=4.978805","-o","sz:error_bound_mode_str=abs"],capture_output=True,text=True)
        print(process.stdout)
        print(process.stderr)
        m=re.search('time:compress_many <uint32> = (\d+)',process.stderr)
        p=re.search('time:decompress_many <uint32> = (\d+)',process.stderr)
    ...
    i=0
    while i<30:
        process=subprocess.run(["/usr/bin/time","pressio","-i","V-98x1200x1200.dat","-m","time","-M","all","-d","141120000","-t","float","sz","-o","sz:abs_err_bound=29.87283","-o","sz:error_bound_mode_str=abs"],capture_output=True,text=True)
        print(process.stdout)
        print(process.stderr)
        m=re.search('time:compress_many <uint32> = (\d+)',process.stderr)
        p=re.search('time:decompress_many <uint32> = (\d+)',process.stderr)
