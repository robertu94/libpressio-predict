...
list5d=[]
list5m=[]
with open('sz_libpressio.csv','w',newline='') as csvfile:
    ...
    i=0
    while i<30:
        process=subprocess.run(["/usr/bin/time","pressio","-i","CLOUDf44.bin","-m","time","-M","all","-d","500","-d","500","-d","100","-t","float","sz","-o","sz:abs_err_bound=0.000000227631","-o","sz:error_bound_mode_str=abs"],capture_output=True,text=True)
    ...
    i=0
    while i<30:
        process=subprocess.run(["/usr/bin/time","pressio","-i","CLOUDf44.bin","-m","time","-M","all","-d","500","-d","500","-d","100","-t","float","sz","-o","sz:abs_err_bound=0.0000227631","-o","sz:error_bound_mode_str=abs"],capture_output=True,text=True)
        print(process.stdout)
        print(process.stderr)
        m=re.search('time:compress_many <uint32> = (\d+)',process.stderr)
        p=re.search('time:decompress_many <uint32> = (\d+)',process.stderr)
    ...
    i=0
    while i<30:
        process=subprocess.run(["/usr/bin/time","pressio","-i","CLOUDf44.bin","-m","time","-M","all","-d","500","-d","500","-d","100","-t","float","sz","-o","sz:abs_err_bound=0.00004553","-o","sz:error_bound_mode_str=abs"],capture_output=True,text=True)
        print(process.stdout)
        print(process.stderr)
        m=re.search('time:compress_many <uint32> = (\d+)',process.stderr)
        p=re.search('time:decompress_many <uint32> = (\d+)',process.stderr)
    ...
    i=0 
    while i<30:
        process=subprocess.run(["/usr/bin/time","pressio","-i","CLOUDf44.bin","-m","time","-M","all","-d","500","-d","500","-d","100","-t","float","sz","-o","sz:abs_err_bound=0.00011382","-o","sz:error_bound_mode_str=abs"],capture_output=True,text=True)
        print(process.stdout)
        print(process.stderr)
        m=re.search('time:compress_many <uint32> = (\d+)',process.stderr)
        p=re.search('time:decompress_many <uint32> = (\d+)',process.stderr)
    ...
    i=0
    while i<30:
        process=subprocess.run(["/usr/bin/time","pressio","-i","CLOUDf44.bin","-m","time","-M","all","-d","500","-d","500","-d","100","-t","float","sz","-o","sz:abs_err_bound=0.00068289","-o","sz:error_bound_mode_str=abs"],capture_output=True,text=True)
        print(process.stdout)
        print(process.stderr)
        m=re.search('time:compress_many <uint32> = (\d+)',process.stderr)
        p=re.search('time:decompress_many <uint32> = (\d+)',process.stderr)
