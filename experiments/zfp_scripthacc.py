...
list5m=[]
list5m2=[]
with open('zfp_testhacc.csv','w',newline='') as csvfile:
    ...
    writer.writeheader()
    i=0
    dimension=numpy.fromfile("/home/vmalvos/datasets/hacc/vx.f32",dtype=numpy.float32).shape
    print(dimension)
    i=0
    while i<30:
        process=subprocess.run(["/usr/bin/time","zfp","-i","vx.f32","-z","vx.f32.zfp","-1","280953867","-f","-a","0.690875"],capture_output=True,text=True)
    ...
    i=0 
    while i<30:
        process=subprocess.run(["/usr/bin/time","zfp","-z","vx.f32.zfp","-o","vx.f32.dec","-1","280953867","-f","-a","0.690875"],capture_output=True,text=True)
        print(process.stdout)
        print(process.stderr)
    ...
    i=0
    while i<30:
        process=subprocess.run(["/usr/bin/time","zfp","-i","vx.f32","-z","vx.f32.zfp","-1","280953867","-f","-a","69.0875"],capture_output=True,text=True)
        print(process.stdout)
        print(process.stderr)
        m=re.search('Compression time: (\d.+)',process.stdout)
        o=re.search('(\d+)maxresident',process.stderr)
    ...
    i=0 
    while i<30:
        process=subprocess.run(["/usr/bin/time","zfp","-z","vx.f32.zfp","-o","vx.f32.dec","-1","280953867","-f","-a","69.0875"],capture_output=True,text=True)
        print(process.stdout)
        print(process.stderr)
        p=re.search('Decompression Time: (\d.+)',process.stdout)
        q=re.search('(\d+)maxresident',process.stderr)
    ...
    i=0
    while i<30:
        process=subprocess.run(["/usr/bin/time","zfp","-i","vx.f32","-z","vx.f32.zfp","-1","280953867","-f","-a","138.175"],capture_output=True,text=True)
        print(process.stdout)
        print(process.stderr)
        m=re.search('Compression time: (\d.+)',process.stdout)
        o=re.search('(\d+)maxresident',process.stderr)
    ...
    i=0 
    while i<30:
        process=subprocess.run(["/usr/bin/time","zfp","-z","vx.f32.zfp","-o","vx.f32.dec","-1","280953867","-f","-a","138.175"],capture_output=True,text=True)
        print(process.stdout)
        print(process.stderr)
        p=re.search('Decompression Time: (\d.+)',process.stdout)
        q=re.search('(\d+)maxresident',process.stderr)
    ...
    i=0
    while i<30:
        process=subprocess.run(["/usr/bin/time","zfp","-i","vx.f32","-z","vx.f32.zfp","-1","280953867","-f","-a","345.4375"],capture_output=True,text=True)
        print(process.stdout)
        print(process.stderr)
        m=re.search('Compression time: (\d.+)',process.stdout)
        o=re.search('(\d+)maxresident',process.stderr)
    ...
    i=0 
    while i<30:
        process=subprocess.run(["/usr/bin/time","zfp","-z","vx.f32.zfp","-o","vx.f32.dec","-1","280953867","-f","-a","345.4375"],capture_output=True,text=True)
        print(process.stdout)
        print(process.stderr)
        p=re.search('Decompression Time: (\d.+)',process.stdout)
        q=re.search('(\d+)maxresident',process.stderr)
    ...
    i=0
    while i<30:
        process=subprocess.run(["/usr/bin/time","zfp","-i","vx.f32","-z","vx.f32.zfp","-1","280953867","-f","-a","2072.625"],capture_output=True,text=True)
        print(process.stdout)
        print(process.stderr)
        m=re.search('Compression time: (\d.+)',process.stdout)
        o=re.search('(\d+)maxresident',process.stderr)
    ...
    i=0 
    while i<30:
        process=subprocess.run(["/usr/bin/time","zfp","-z","vx.f32.zfp","-o","vx.f32.dec","-1","280953867","-f","-a","2072.625"],capture_output=True,text=True)
        print(process.stdout)
        print(process.stderr)
        p=re.search('Decompression Time: (\d.+)',process.stdout)
        q=re.search('(\d+)maxresident',process.stderr)
