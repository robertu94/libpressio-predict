...
list5m=[]
list5m2=[]
with open('sz_testhacc.csv','w',newline='') as csvfile:
    ...
    i=0
    while i<30:
        process=subprocess.run(["/usr/bin/time","sz","-z","-i","vx.f32","-M","ABS","-A","0.690875","-f","-1","280953867"],capture_output=True,text=True)
    ...
    i=0
    while i<30:
        process=subprocess.run(["/usr/bin/time","sz","-x","-f","-M","ABS","-A","0.690875","-1","280953867","-s","vx.f32.sz"],capture_output=True,text=True)
        print(process.stdout)
        print(process.stderr)
    ...
    i=0
    while i<30:
        process=subprocess.run(["/usr/bin/time","sz","-z","-i","vx.f32","-M","ABS","-A","69.0875","-f","-1","280953867"],capture_output=True,text=True)
        print(process.stdout)
        print(process.stderr)
        m=re.search('compression time = (\d.+)',process.stdout)
        o=re.search('(\d+)maxresident',process.stderr)
    ...
    i=0
    while i<30:
        process=subprocess.run(["/usr/bin/time","sz","-x","-f","-M","ABS","-A","69.0875","-1","280953867","-s","vx.f32.sz"],capture_output=True,text=True)
        print(process.stdout)
        print(process.stderr)
        p=re.search('decompression time = (\d.+) seconds',process.stdout)
        q=re.search('(\d+)maxresident',process.stderr)
    ...
    i=0
    while i<30:
        process=subprocess.run(["/usr/bin/time","sz","-z","-i","vx.f32","-M","ABS","-A","138.175","-f","-1","280953867"],capture_output=True,text=True)
        print(process.stdout)
        print(process.stderr)
        m=re.search('compression time = (\d.+)',process.stdout)
        o=re.search('(\d+)maxresident',process.stderr)
    ...
    i=0
    while i<30:
        process=subprocess.run(["/usr/bin/time","sz","-x","-f","-M","ABS","-A","138.175","-1","280953867","-s","vx.f32.sz"],capture_output=True,text=True)
        print(process.stdout)
        print(process.stderr)
        p=re.search('decompression time = (\d.+) seconds',process.stdout)
        q=re.search('(\d+)maxresident',process.stderr)
    ...
    i=0
    while i<30:
        process=subprocess.run(["/usr/bin/time","sz","-z","-i","vx.f32","-M","ABS","-A","345.4375","-f","-1","280953867"],capture_output=True,text=True)
        print(process.stdout)
        print(process.stderr)
        m=re.search('compression time = (\d.+)',process.stdout)
        o=re.search('(\d+)maxresident',process.stderr)
    ...
    i=0
    while i<30:
        process=subprocess.run(["/usr/bin/time","sz","-x","-f","-M","ABS","-A","345.4375","-1","280953867","-s","vx.f32.sz"],capture_output=True,text=True)
        print(process.stdout)
        print(process.stderr)
        p=re.search('decompression time = (\d.+) seconds',process.stdout)
        q=re.search('(\d+)maxresident',process.stderr)
    ...
    i=0
    while i<30:
        process=subprocess.run(["/usr/bin/time","sz","-z","-i","vx.f32","-M","ABS","-A","2072.625","-f","-1","280953867"],capture_output=True,text=True)
        print(process.stdout)
        print(process.stderr)
        m=re.search('compression time = (\d.+)',process.stdout)
        o=re.search('(\d+)maxresident',process.stderr)
    ...
    i=0
    while i<30:
        process=subprocess.run(["/usr/bin/time","sz","-x","-f","-M","ABS","-A","2072.625","-1","280953867","-s","vx.f32.sz"],capture_output=True,text=True)
        print(process.stdout)
        print(process.stderr)
        p=re.search('decompression time = (\d.+) seconds',process.stdout)
        q=re.search('(\d+)maxresident',process.stderr)
